...
class CNNSentenceEncoder(nn.Module):
    ...
    def tokenize(self, raw_tokens, pos_head, pos_tail):
        # token -> index
        indexed_tokens = []
        #print(raw_tokens)   a mondat
        for token in raw_tokens:
            token = token.lower()
            if token in self.word2id:
                indexed_tokens.append(self.word2id[token])
            else:
                indexed_tokens.append(self.word2id['[UNK]'])     # HA NINCS BENNE A SZÓTÁRBAN
        
        # padding  HA A HOSSZA TUL NAGY
        ...
        #print(indexed_tokens)   adott dimenzióban az szó id
        #print(pos1)
        #print(pos2)
        #print(mask)
...
######################################################################### SAJAT
class FasttextSentenceEncoder(nn.Module):
    ...
    def __init__(self, max_length, word_embedding_dim=50, 
        ...
        self.hidden_size = hidden_size
        self.max_length = max_length
        self.fasttext_model=fasttext.load_model('./pretrain/fasttext/bintext/wiki.hu.bin')
    ...
    def tokenize(self, raw_tokens, pos_head, pos_tail):
        ...
        for token in raw_tokens:
            token = token.lower()
            indexed_tokens.append(self.fasttext_model.get_word_vector(token))
...
class BERTPAIRMULTISentenceEncoder(nn.Module):
...
#########################################################################################################
class BertSentenceEncoderOWN(nn.Module):
